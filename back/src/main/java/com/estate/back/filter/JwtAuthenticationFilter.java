...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final UserRepository userRepository;        // userRepository 의존성주입
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            // (검증결과에 대한)접근 주체에 권한을 확인
            UserEntity userEntity = userRepository.findByUserId(userId);                // userEntity를 userRepository를 통해 가져옴
            if (userEntity == null) {                                           // null이 오는지 확인
                filterChain.doFilter(request, response);
                return;
            }
